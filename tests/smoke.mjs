...
try{
 ...
 await page.getByRole('button',{name:/Catálogo Oferta/}).first().click();
 await page.getByText('Ajustar dados do criativo (opcional)',{exact:true}).click();
 await page.getByRole('button',{name:'Capturar outro frame'}).click();
 ...
 assert.equal(updated.match(/<g:price>(.*?)<\/g:price>/)[1],feed.match(/<g:price>(.*?)<\/g:price>/)[1]);
 assert.equal(updated.match(/<g:id>(.*?)<\/g:id>/)[1],feed.match(/<g:id>(.*?)<\/g:id>/)[1]);
