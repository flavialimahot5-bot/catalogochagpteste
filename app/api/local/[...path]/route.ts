...
export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}) {
 ...
 if(!/^(media|feeds)\/[a-zA-Z0-9_-]+\.(jpg|mp4|mov|webm|xml|csv)$/.test(key)) return new Response('Not found',{status:404});
 try {
  const bytes=await read(key);
  const mime:Record<string,string>={jpg:'image/jpeg',mp4:'video/mp4',mov:'video/quicktime',webm:'video/webm',xml:'application/xml; charset=utf-8',csv:'text/csv; charset=utf-8'};
